...
void M_Init(void)
{
	...
	quitmsg[QUIT2MSG6] = M_GetText("Hit the SELECT key, Sonic!\nThe SELECT key!\n\n(Press START to quit)");
	...
	quitmsg[QUIT3MSG2] = M_GetText("Press SELECT to unlock\nthe Ultimate Cheat!\n\n(Press START to quit)");
	quitmsg[QUIT3MSG3] = M_GetText("Why don't you go back and try\njumping on that house to\nsee what happens?\n\n(Press START to quit)");
	quitmsg[QUIT3MSG4] = M_GetText("Every time you press START, an\nSRB2 Developer cries...\n\n(Press START to quit)");
...
static void M_DrawVideoMode(void)
{
	...
	if (vidm_testingmode > 0)
	{
		...
		#ifdef GCW0
			"Press START again to keep this mode");
		#else
